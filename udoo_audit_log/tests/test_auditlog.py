...
class TestFieldRemoval(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_model = (
            cls.env['ir.model']
            .sudo()
            .create([{'name': 'x_test_model', 'model': 'x_test.model', 'state': 'manual'}])
        )
...
class AuditLogRuleTestForUserFields(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.contact_model_id = cls.env['ir.model'].search([('model', '=', 'res.partner')]).id

        # get phone field id
        cls.fields_to_exclude_ids = (
            cls.env['ir.model.fields'].search([('model', '=', 'res.partner'), ('name', '=', 'phone')]).id
        )
    ...
    def test_03_AuditlogFull_user_exclude_write_log(self):
        ...
        partner_form = Form(self.testpartner1.with_user(self.user.id).with_context(tracking_disable=True))
...
class AuditLogRuleTestForUserModel(TransactionCase):
    ...
    def test_01_AuditlogFull_field_group_write_log(self):
        ...
        self.user.with_context(tracking_disable=True).write({'groups_id': [(4, self.group.id)]})
        # Checking log is created for testpartner1
        write_log_record = self.auditlog_log.search(
            [
                ('model_id', '=', self.auditlog_rule.model_id.id),
