...
class AuditlogRule(models.Model):
    ...
    def _patch_methods(self):
        ...
        for rule in self:
            if rule.state != 'subscribed' or not self.pool.get(rule.model_id.model or rule.model_model):
                continue
            ...
            if rule.log_create and not hasattr(model_model, check_attr):
                patch(model_model, 'create', rule._make_create())
    ...
    def get_auditlog_fields(self, model):
        ...
        override this.
        """
        return list(n for n, f in model._fields.items() if (not f.compute and not f.related) or f.store)
    ...
    def _make_write(self):
        ...
        def write_full(self, vals, **kwargs):
            ...
            fields_list = rule_model.get_auditlog_fields(self)
            old_values = {
                d['id']: d for d in self.sudo().with_context(prefetch_fields=False).read(fields_list)
            }
    ...
    def _get_field(self, model, field_name):
        ...
        if field_name not in cache.get(model.model, {}):
            ...
            field = field_model.search([('model_id', 'in', all_model_ids), ('name', '=', field_name)])
    ...
    def _create_log_line_on_read(self, log, fields_list, read_values, fields_to_exclude):
    ...
    def _create_log_line_on_write(self, log, fields_list, old_values, new_values, fields_to_exclude):
    ...
    def _create_log_line_on_create(self, log, fields_list, new_values, fields_to_exclude):
