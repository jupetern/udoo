...
class AuditlogLog(models.Model):
    ...
    model_id = fields.Many2one('ir.model', string='Model', index=True, ondelete='set null')
    ...
    http_session_id = fields.Many2one('auditlog.http.session', string='Session', index=True)
    http_request_id = fields.Many2one('auditlog.http.request', string='HTTP Request', index=True)
    log_type = fields.Selection([('full', 'Full log'), ('fast', 'Fast log')], string='Type')
...
class AuditlogLogLine(models.Model):
    ...
    field_id = fields.Many2one('ir.model.fields', ondelete='set null', string='Field', index=True)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            vals.update({'field_name': field.name, 'field_description': field.field_description})
        return super().create(vals_list)
