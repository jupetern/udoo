...
class XLSXTemplate(models.Model):
    ...
    def _compute_result_field(self):
        for rec in self:
            rec.result_field = ('x_%s_results' % rec.id) if rec.result_model_id else False

    @api.constrains('redirect_action', 'res_model')
    def _check_action_model(self):
        for rec in self:
            if rec.res_model and rec.redirect_action and rec.res_model != rec.redirect_action.res_model:
                raise ValidationError(
                    _('The selected redirect action is ' 'not for model %s') % rec.res_model
                )
