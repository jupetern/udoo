...
class XLSXExport(models.AbstractModel):
    ...
    def _spec_attrib(self):
        return ('_NAME_', '_HEAD_')
    ...
    def _get_line_vals(self, record, line_field, fields):
        ...
        for line in lines:
            for field in pair_fields:  # (field, raw_field)
                ...
                # If def_style and style then concat def_style before style them else use def_style
                def_style = field_style_dict[field[0]]
                if def_style:
                    if style:
                        style = f'{def_style};{style}'
                    else:
                        style = def_style
    ...
    @api.model
    def _fill_workbook_data(self, workbook, record, export_dict, fname):
        """Fill data from record with style in data_dict to workbook"""
        if not record or not export_dict:
            return
        try:
            for sheet_name in export_dict:
                ws = export_dict[sheet_name] or self._context.get(f'export_{fname}.{sheet_name}')
                st = False
                ...
                mg_he = self._fill_head(ws, st, record)
                mg_ln = self._fill_lines(ws, st, record)

                # Supports cell right/bottom expansion
                for cell_range in mg_he + mg_ln:
                    st.merge_cells(range_string=cell_range)

                # Assign sheet name if defined
                sheet_name = ws.get('_NAME_')
                if sheet_name:
                    st.title = sheet_name
    ...
    def _fill_head(self, ws, st, record):
        ...
        for rc, field in ws.get('_HEAD_', {}).items():
            ...
            if field_style and fc:  # has style and pass style_cond
                ...
                co.fill_cell_style(st[rc], field_styles, styles)
                merge_ranges.append(self._get_merge_range(col, row, field_styles))
        return merge_ranges
    ...
    def _fill_lines(self, ws, st, record):
        ...
        for line_field in line_fields:
            if line_field in self._spec_attrib():
                continue
            ...
            for rc, field in ws.get(line_field, {}).items():
                ...
                if is_extend and not rows_inserted:
                    rows_inserted = True
                    cursor = row + 1
                    n_inserted = len(vals[field]) - 1
                    st.insert_rows(cursor, n_inserted)

                    st: Worksheet = st
                    for merged_cell in st.merged_cells.ranges:
                        if merged_cell.min_row >= cursor:
                            omc: MergedCellRange = merged_cell
                            omc.shift(0, n_inserted)
                ...
                for row_val, style in vals[field]:
                    ...
                    if style:
                        styles = self.env['xlsx.styles'].get_openpyxl_styles()
                        field_styles = style.split(';') if style else []
                        co.fill_cell_style(st[new_rc], field_styles, styles)
                        merge_ranges.append(self._get_merge_range(col, new_row, field_styles))
                    i += 1
                ...
                if f and new_row > 0:
                    ...
                    styles = self.env['xlsx.styles'].get_openpyxl_styles()
                    co.fill_cell_style(st[f_rc], field_styles, styles)
                cont_row = cont_row < new_row and new_row or cont_row

        return merge_ranges

    def _get_merge_range(self, col, row, field_styles):
        """Return the cell ranges need to merge"""
        colspan, rowspan = 0, 0
        for pair in [f.split('=') for f in field_styles if f.startswith(('rspan', 'bspan'))]:
            (att, val) = pair
            if att == 'rspan':
                colspan = int(val)
            if att == 'bspan':
                rowspan = int(val)
        return '{}{}:{}{}'.format(
            col, row, get_column_letter(column_index_from_string(col) + colspan), row + rowspan
        )
    ...
    def export_xlsx(self, template, res_model, res_ids):
        ...
        fx_index = template.fname.rfind('.xlsx')
        fx_name = template.fname[:fx_index]
        ...
        for record in records:
            ...
            wb: Workbook = load_workbook(ftemp)
            os.remove(ftemp)

            # Dynamic excel export instruction
            method = getattr(record, '_o_export_dict', None)
            extra_ctx = record._o_export_dict() if callable(method) else {}
            self.with_context({**self._context, **extra_ctx})._fill_workbook_data(
                wb, record, export_dict, fx_name
            )
            # Return file as .xlsx
            content = BytesIO()
            wb.save(content)
