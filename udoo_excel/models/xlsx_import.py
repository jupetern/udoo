...
class XLSXImport(models.AbstractModel):
    ...
    def _get_field_type(self, model, field):
        try:
            ...
            raise ValidationError(_('Invalid declaration, %s has no valid field type') % field) from exc
    ...
    def _import_record_data(self, import_file, record, data_dict):
        ...
        try:
            ...
            xml_id = (
                record and self.get_external_id(record) or '{}.{}'.format('__udoo_excel__', uuid.uuid4())
            )
            ...
            raise ValidationError(_('Invalid file style, only .xls or .xlsx file allowed')) from exc
        except Exception as e:
            raise e
    ...
    def import_xlsx(self, import_file, template, res_model=False, res_id=False):
        ...
        if not data_dict.get('__IMPORT__'):
            raise ValidationError(_("No data_dict['__IMPORT__'] in template %s") % template.name)
        if record:
