...
def get_field_style_cond(field):
    """i..e, 'field#?object.partner_id and #{format=text}#{font=bold} or #{}?'"""
    if field and '#?' in field and '?' in field:
        i = field.index('#?')
        j = field.index('?', i + 2)
        cond = field[i + 2 : j]
...
def fill_cell_style(field, field_styles, styles):
    for f in field_styles:
        (key, value) = f.split('=')
        """
            rspan: num of spanned column, right
            bspan: num of spanned row, bottom
        """
        if key in ['rspan', 'bspan']:
            continue
        if key not in styles.keys():
            raise ValidationError(_('Invalid style type %s') % key)
        if value.lower() not in styles[key].keys():
            raise ValidationError(
                _('Invalid value %(value)s for style type %(key)s') % {'value': value, 'key': key}
            )
...
def _get_cell_value(cell, field_type=False):
    ...
    if field_type in ['date', 'datetime']:
        ...
        if ctype in ('xldate', 'number'):
            ...
            date = dt(*time_tuple)
            value = date.strftime('%Y-%m-%d %H:%M:%S') if is_datetime else date.strftime('%Y-%m-%d')
        else:
            value = cell.value
...
def _add_id_column(file_txt):
    ...
    for line in file_txt.split('\n'):
        if line and i == 0:
            ...
            line = f'__udoo_excel__.{uuid.uuid4()},{line}'
        txt_lines.append(line)
        i += 1
    file_txt = '\n'.join(txt_lines)
    return file_txt
