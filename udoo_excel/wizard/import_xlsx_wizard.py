...
class ImportXLSXWizard(models.TransientModel):
    ...
    fname = fields.Char(string='Template Name', related='template_id.fname', readonly=True)
    ...
    def view_init(self, fields_list):
        ...
        if self._context.get('template_context', False):
            ...
            for key, value in template_context.items():
                if (
                    key not in record
                    or (record._fields[key].type == 'many2one' and record[key].id or record[key]) != value
                ):
                    valid = False
                    messages.append(_('This import action is not usable ' 'in this document context'))
                    break
        if not valid:
    ...
    def default_get(self, fields):
        ...
        for template in templates:
            if not template.datas:
                act = self.env.ref('udoo_excel.action_xlsx_template')
    ...
    def action_import(self):
        ...
        res_ids = []
        if self.import_file:
            record = Import.import_xlsx(self.import_file, self.template_id, self.res_model, self.res_id)
            res_ids = [record.id]
