...
class ReportController(report.ReportController):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        if converter == 'excel':
            ...
            excel, report_name = report.with_context(**context)._render_excel(docids, data=data)
    ...
    def report_download(self, data, context=None):
        ...
        try:
            ...
            if docids:
                return self.report_routes(reportname, docids=docids, converter='excel', context=context)
            data = dict(url_decode(url.split('?')[1]).items())
            if 'context' in data:
                context, data_context = json.loads(context or '{}'), json.loads(data.pop('context'))
                context = json.dumps({**context, **data_context})
            return self.report_routes(reportname, converter='excel', context=context, **data)
